...
class _PersonalAdditemState extends State<PersonalAdditem> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: personalAddItemWidget(
        ...
        onItemPriceChange: (String text, int index) =>
            _onItemPriceChange(text, index),
  ...
  void _onSelectDelivery(bool checkStoreSuggest) {
    ...
    if (_itemsMapList.isNotEmpty) {
      if ((_itemsMapList[_itemsMapList.length - 1]["name"]
      ...
      if ((_itemsMapList[_itemsMapList.length - 1]["price"]
              as TextEditingController)
          .text
          .isEmpty) {
        toastContainer(
  ...
  void _onStartAddingItem() {
    ...
    setState(() {});
    (_itemsMapList[_itemsMapList.length - 1]["nameFocus"] as FocusNode)
        .requestFocus();
    _errorInBuildup = false;
  }
}
