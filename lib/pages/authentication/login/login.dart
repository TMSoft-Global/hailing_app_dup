...
class _LoginPageState extends State<LoginPage> {
      ...
      if (!_phoneController.text.startsWith("+233 ")) {
        ...
        if (previousTextLength > 5) {
          _phoneController.selection =
              TextSelection.collapsed(offset: previousTextLength);
        }
      }
    });
  }
  ...
  Future<void> _onLogin({
    ...
    if (httpResult["ok"]) {
      bool accountExit = httpResult["data"]["data"]["userExists"] != null &&
          httpResult["data"]["data"]["userExists"];
      ...
      try {
        if (httpResult["data"] != null &&
            !httpResult["data"]["data"]["emailVerified"]) {
          if (!mounted) return;
          Navigator.of(context).push(
            MaterialPageRoute(
              builder: (context) => OtpVerification(
